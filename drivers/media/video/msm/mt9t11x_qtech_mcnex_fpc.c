...
#define MT9T11X_CAMIO_MCLK  54000000    //PCLK=48MHz, set by PLL setting
...
#define MT9T11X_GPIO_SWITCH_CTL     30
#define MT9T11X_GPIO_SWITCH_VAL     0
#else
...
#ifdef FIXED_COMPATIBILITY_WITH_ISO
/*
 * ISO Setting
 */
...
static int mt9t11x_sensor_dev_probe(const struct msm_camera_sensor_info *pinfo)
{
    ...
    /*
      * Sensor Clock Switch
      */
#if defined(CONFIG_MACH_RAISE) || defined(CONFIG_MACH_V9) || defined(CONFIG_MACH_SAILBOAT)
    ...
    model_id = 0x0000;
    rc = mt9t11x_i2c_read(mt9t11x_client->addr, REG_MT9T11X_MODEL_ID, &model_id, WORD_LEN);
	pr_err("%s: rc=0x%x,model_id = 0x%x\n", __func__, rc,model_id);
    if (rc < 0)
    {
        return rc;
    }
...
static int mt9t11x_sensor_init(const struct msm_camera_sensor_info *data)
{
    ...
    pr_err("msm_camera_clk_switch ok");
#else
    // do nothing
#endif    
    /* Input MCLK */
...
static int mt9t11x_sensor_config(void __user *argp)
{
    ...
    switch (cfg_data.cfgtype)
    {
        ...
        case CFG_SET_WB:
        {
            rc = mt9t11x_set_wb(cfg_data.cfg.wb_val);
        }
        break;
...
static int mt9t11x_sensor_release_internal(void)
{
...
#if defined(CONFIG_MACH_V9) || defined(CONFIG_MACH_ROAMER) || defined(CONFIG_MACH_SAILBOAT)
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x098E, 0x3003, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    ...
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x098E, 0x6005, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    ...
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0992, 0x1800, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0994, 0x8000, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0996, 0x0000, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x098E, 0x6003, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0990, 0x8001, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    rc = mt9t11x_i2c_write(mt9t11x_client->addr, 0x0606, 0x100F, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
